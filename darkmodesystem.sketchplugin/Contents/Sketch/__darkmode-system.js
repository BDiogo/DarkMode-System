...
function DarkModeSystem(context, type) {
  ...
  var document = sketch__WEBPACK_IMPORTED_MODULE_0___default.a.getSelectedDocument(); // Page variables

  page = context.document.currentPage(), pageInfo = page.userInfo();
  var fieldHeight = 22,
      fieldWidth = 60,
      labelHeight = 16,
      leftColWidth = 120,
  ...
  if (type && type == "darkmode") {
    ...
    var s_styles = Settings.settingForKey('Styles'); //Settings.setDocumentSettingForKey(document, 'Font', groupdefaultFont.indexOfSelectedItem());
    //create Modal
  ...
  // ------- ------- ------- -------  
    ...
    // ------- ------- ------- -------  
      ...
      // ------- ------- ------- -------  
          ...
          var col = windowWidth / 2 - 60; //LABELS
          ...
          if (responseCode == 1000) {
            ...
            sketch__WEBPACK_IMPORTED_MODULE_0___default.a.UI.message(' Updated Sucessfully');
          } else return false;
        } // ------------------------------------------------
...
function reviewSettings(Settings, document) {
  var all_settings = Settings.settingForKey('Styles');
...
function syncLayerwithStyles_best(Settings, sketch, document, page) {
  ...
  var container_min_height = 200; // DO CONTAINERS
  ...
  for (var i = 0; i < all_shapesPaths.length; i++) {
    if (all_shapesPaths[i].frame.width > container_min_width && all_shapesPaths[i].frame.height > container_min_height && all_shapesPaths[i].shapeType == "Rectangle") {
      ...
      if (!hasStyle || hasStyle && isOutOfSync) {
        var hasMatchingStyle = elementHasMatchingStyle(layer_styles, all_shapesPaths[i].style); // get setting with macthcing style
        ...
        if (!hasMatchingStyle || hasmach_butnot) {
          ...
          layer_styles = addNewStyle(document, "Shape", "Layer", all_shapesPaths[i].style, count_shapePath_added, layer_styles); //update element
  ...
  for (var i = 0; i < all_shapesPaths.length; i++) {
    if (all_shapesPaths[i].frame.width < container_min_width || all_shapesPaths[i].frame.height < container_min_height || all_shapesPaths[i].shapeType != "Rectangle") {
      ...
      if (!hasStyle || hasStyle && isOutOfSync) {
        var hasMatchingStyle = elementHasMatchingStyle(layer_styles, all_shapesPaths[i].style); // is has matching style but one is hover a colored container and the other is not
  ...
  for (var i = 0; i < all_shapes.length; i++) {
    ...
    if (!hasStyle || hasStyle && isOutOfSync) {
      var hasMatchingStyle = elementHasMatchingStyle(layer_styles, all_shapes[i].style); // is has matching style but one is hover a colored container and the other is not
  ...
  for (var i = 0; i < all_texts.length; i++) {
    ...
    if (!hasStyle || hasStyle && isOutOfSync) {
      var hasMatchingStyle = elementHasMatchingStyle(text_styles, all_texts[i].style); // is has matching style but one is hover a colored container and the other is not
...
function createDarkModeStyle(Settings, document, styles_layer, styles_text, new_styles) {
  sketch__WEBPACK_IMPORTED_MODULE_0___default.a.UI.message(' Doing step 2,  Creating Dark Mode......');
  console.log(".........................................");
  console.log("Step 2 - Create Dark Mode Layer Styles...");
  var percentage_layer = 0;
  var percentage_text = 0;
  var s_styles = Settings.settingForKey('Styles'); //layer color
